...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!incidents || incidents.length === 0) {
      ...
      const transformedFallbackIncidents = fallbackIncidents.map(incident => ({
        id: incident.id,
    ...
    return NextResponse.json(
      { error: 'Failed to fetch recent incidents', details: error instanceof Error ? error.message : 'Unknown error' },
      { status: 500 }
    );
  }
}
