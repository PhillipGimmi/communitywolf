...
// Timeout constants (in milliseconds) - Optimized for Vercel's 30s limit
const OVERALL_TIMEOUT = 25000; // 25 seconds total timeout (leaving 5s buffer for Vercel)
const SERPAPI_TIMEOUT = 10000; // 10 seconds for SerpAPI
const OPENROUTER_TIMEOUT = 12000; // 12 seconds for OpenRouter
...
async function generateIntelligentAlertsWithStructuredOutput(
  ...
  try {
    ...
    try {
      searchResults = await performSerpAPISearch(searchQuery, abortSignal);
    } catch (serpError) {
      console.warn('⚠️ SerpAPI search failed, using fallback data:', serpError);
      // Create fallback search results to prevent complete failure
      searchResults = createFallbackSearchResults(location);
    }
    
    if (searchResults.length === 0) {
      console.warn('⚠️ No search results available, using fallback data');
      searchResults = createFallbackSearchResults(location);
    }
    
    console.log('✅ STEP 2 COMPLETE: Search found', searchResults.length, 'results with URLs');
    ...
    try {
      // Create a timeout promise for OpenRouter
      const openRouterTimeoutPromise = new Promise<never>((_, reject) => {
        setTimeout(() => reject(new Error('OpenRouter request timed out')), OPENROUTER_TIMEOUT);
      });
      
      // Create the fetch promise with abort signal
      const openRouterFetchPromise = fetch('https://openrouter.ai/api/v1/chat/completions', {
        method: 'POST',
        headers: {
        ...
        body: JSON.stringify({
          ...
          messages: [
            {
              role: 'system',
              content: 'You are a safety intelligence analyst. Generate safety alerts using ONLY the provided search results. Return valid JSON only. Keep responses concise.'
            },
            {
              role: 'user',
              content: context
          ...
          temperature: 0.1,
          max_tokens: 800, // Reduced from 1500 to speed up processing
      ...
      const response = await Promise.race([openRouterFetchPromise, openRouterTimeoutPromise]);
      ...
      console.warn('⚠️ OpenRouter failed, using fallback alerts:', openRouterError);
      // Create fallback alerts to prevent complete failure
      generatedAlerts = createFallbackAlerts(location, searchResults);
    }
...
async function performSerpAPISearch(query: string, abortSignal: AbortSignal): Promise<SearchResult[]> {
  try {
    ...
    // Limit to 5 results to reduce processing time
    ...
    // Add organic results (limit to 5)
    if (data.organic_results && Array.isArray(data.organic_results)) {
      console.log('   Found', data.organic_results.length, 'organic results');
      data.organic_results.slice(0, 5).forEach((result: { title?: string; link?: string; source?: string; snippet?: string }) => {
    ...
    // Add news results if available (limit to 5 total)
    if (data.news_results && Array.isArray(data.news_results) && results.length < 5) {
      const remainingSlots = 5 - results.length;
      console.log('   Found', data.news_results.length, 'news results, adding', remainingSlots);
      data.news_results.slice(0, remainingSlots).forEach((result: { title?: string; link?: string; source?: string; snippet?: string }) => {
        if (result.title && result.link) {
          results.push({
            title: result.title,
            url: result.link,
    ...
    console.log('   SerpAPI found', results.length, 'total results (limited to 5)');
...
function buildOpenRouterContext(
  ...
  const reportSummary = recentReports.length > 0 
    ? `Recent crime reports: ${recentReports.map(r => formatReportSummary(r)).join(', ')}`
  ...
  const searchResultsText = searchResults.length > 0 
    ? searchResults.map((result, index) => formatSearchResult(result, index + 1)).join('\n\n')
...
function formatReportSummary(report: RecentReport): string {
  return `${report.type} (${report.severity})`;
}
...
function validateAlert(alert: Partial<GeneratedAlert>): GeneratedAlert {
  ...
  return {
    ...
    severity: alert.severity && ALERT_SEVERITIES.includes(alert.severity) 
      ? alert.severity 
      : 'medium',
    ...
    alertType: alert.alertType && ALERT_TYPES.includes(alert.alertType)
      ? alert.alertType
      : 'safety',
    keywords: alert.keywords ?? [],
...
function extractPartialAlertsFromContent(content: string): GeneratedAlert[] {
  ...
  // Build regex pattern in parts to reduce complexity
  const buildAlertRegex = () => {
    const patterns = [
      '"id":\\s*"([^"]+)"',
      '"title":\\s*"([^"]+)"',
      '"shortDescription":\\s*"([^"]+)"',
  ...
  const alertRegex = buildAlertRegex();
...
function buildPromptTemplate(
  ...
  const template = `Location: ${locationInfo} (${radius}km radius)
...
TASK: Generate EXACTLY 5 safety alerts - one for each search result above.

CRITICAL REQUIREMENTS:
- Generate EXACTLY 5 alerts (one per search result)
...
- Use current timestamp: TIMESTAMP_PLACEHOLDER
...
- longDescription: 2-3 sentence detailed description with context and implications
...
Return ONLY valid JSON in this exact format:
{
  "alerts": [
    ...
    // ... continue for ALL 5 search results
  ]
}`;

  // Replace placeholder with actual timestamp
  return template.replace(/TIMESTAMP_PLACEHOLDER/g, timestamp);
}

function createFallbackSearchResults(location: string): SearchResult[] {
  console.log('🔄 Creating fallback search results for:', location);
  
  // Extract suburb from location
  const parts = location.split(', ');
  const suburb = parts[1] || 'Area';
  
  return [
    {
      title: `Safety Alert - ${suburb} Area`,
      url: 'https://example.com/fallback-safety',
      source: 'Local Safety Database',
      snippet: `General safety information for ${suburb} area residents.`
    },
    {
      title: `Crime Prevention - ${suburb}`,
      url: 'https://example.com/fallback-crime',
      source: 'Community Safety',
      snippet: `Crime prevention tips and community safety guidelines for ${suburb}.`
    },
    {
      title: `Emergency Preparedness - ${suburb}`,
      url: 'https://example.com/fallback-emergency',
      source: 'Emergency Services',
      snippet: `Emergency preparedness information for ${suburb} residents.`
    },
    {
      title: `Neighborhood Watch - ${suburb}`,
      url: 'https://example.com/fallback-watch',
      source: 'Community Watch',
      snippet: `Neighborhood watch program information for ${suburb} area.`
    },
    {
      title: `Local Safety Resources - ${suburb}`,
      url: 'https://example.com/fallback-resources',
      source: 'Safety Resources',
      snippet: `Local safety resources and contact information for ${suburb}.`
    }
  ];
}

function createFallbackAlerts(location: string, searchResults: SearchResult[]): GeneratedAlert[] {
  console.log('🔄 Creating fallback alerts for:', location);
  ...
  return searchResults.map((result, index) => ({
    id: `SA-2025-${String(index + 1).padStart(3, '0')}`,
    title: result.title,
    shortDescription: result.snippet,
    longDescription: `This is a fallback safety alert for ${suburb} area. ${result.snippet} Please stay vigilant and report any suspicious activity to local authorities.`,
    ...
    keywords: ['safety', 'community', suburb.toLowerCase(), 'alert'],
    recommendations: `Stay alert and report any suspicious activity. Keep emergency numbers handy and stay informed about local safety updates.`
  }));
}