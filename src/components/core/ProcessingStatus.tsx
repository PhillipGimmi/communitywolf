...
export function ProcessingStatus({ isActive }: ProcessingStatusProps) {
  ...
  // Extract functions outside useEffect to reduce nesting
  const resetSteps = (prevSteps: ProcessingStep[]) => 
    prevSteps.map(step => ({ ...step, status: 'pending' as const }));

  const updateStepStatus = (prev: ProcessingStep[], index: number, status: ProcessingStep['status']) => 
    prev.map((step, i) => i === index ? { ...step, status } : step);

  const updateStep = useCallback((index: number, status: ProcessingStep['status']) => {
    setSteps(prev => updateStepStatus(prev, index, status));
  }, []);

  const runProcessingSequence = useCallback(async () => {
    // Step 1: SearchAgent
    updateStep(0, 'active');
  ...
  }, [isActive, runProcessingSequence]);
