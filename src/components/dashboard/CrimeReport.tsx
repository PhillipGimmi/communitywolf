  ...
  const handleMapClick = (coordinates: { lat: number; lng: number }, address?: string) => {
    ...
    try {
      ...
      const submissionData = {
        ...
        severity: (reportData.severity_level === 1 ? 'low' : 
                 reportData.severity_level === 2 ? 'medium' : 
                 reportData.severity_level === 3 ? 'high' : 
                 reportData.severity_level === 4 ? 'critical' : 'emergency') as 'low' | 'medium' | 'high' | 'critical' | 'emergency',
